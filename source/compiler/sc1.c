...
static void about(void)
{
  if (strlen(errfname) == 0) {
    setcaption();
    fputs("Usage:   pawncc <filename> [filename...] [options]\n\n", stdout);
    fputs("Options:\n", stdout);
    fputs("         -A<num>  alignment in bytes of the data segment and the stack\n", stdout);
    fputs("         -a       output assembler code\n", stdout);
#if AMX_COMPACTMARGIN > 2
    pc_printf("         -C[+/-]  compact encoding for output file (default=%c)\n", sc_compress ? '+' : '-');
#endif
    fputs("         -c<name> codepage name or number; e.g. 1252 for Windows Latin-1\n", stdout);
    fputs("         -Dpath   active directory path\n", stdout);
    pc_printf("         -d<num>  debugging level (default=-d%d)\n", sc_debug);
    fputs("             0    no symbolic information, no run-time checks\n", stdout);
    fputs("             1    run-time checks, no symbolic information\n", stdout);
    fputs("             2    full debug information and dynamic checking\n", stdout);
    fputs("             3    same as -d2, but implies -O0\n", stdout);
    fputs("         -e<name> set name of error file (quiet compile)\n", stdout);
#if defined __WIN32__ || defined _WIN32 || defined _Windows
    fputs("         -H<hwnd> window handle to send a notification message on finish\n", stdout);
#endif
    fputs("         -i<name> path for include files\n", stdout);
    fputs("         -l       create list file (preprocess only)\n", stdout);
    fputs("         -o<name> set base name of (P-code) output file\n", stdout);
    pc_printf("         -O<num>  optimization level (default=-O%d)\n", pc_optimize);
    fputs("             0    no optimization\n", stdout);
    fputs("             1    JIT-compatible optimizations only\n", stdout);
    fputs("             2    full optimizations\n", stdout);
    fputs("         -p<name> set name of \"prefix\" file\n", stdout);
    pc_printf("         -R[+/-]  add detailed recursion report with call chains (default=%c)\n", pc_recursion ? '+' : '-');
#if !defined SC_LIGHT
    fputs("         -r[name] write cross reference report to console or to specified file\n", stdout);
#endif
    pc_printf("         -S<num>  stack/heap size in cells (default=%d)\n", (int)pc_stksize);
    fputs("         -s<num>  skip lines from the input file\n", stdout);
    ...
    fputs("         -w<num>  disable a specific warning by its number\n", stdout);
    fputs("         -X<num>  abstract machine size limit in bytes\n", stdout);
    fputs("         -XD<num> abstract machine data/stack size limit in bytes\n", stdout);
    fputs("         -G[+/-]  enable/disable include guard to prevent multiple inclusions\n", stdout);
    fputs("         -E[+/-]  turn warnings in to errors\n", stdout);
    fputs("         -\\       use '\\' for escape characters\n", stdout);
    fputs("         -^       use '^' for escape characters\n", stdout);
    ...
    fputs("         sym=val  define constant \"sym\" with value \"val\"\n", stdout);
    fputs("         sym=     define constant \"sym\" with value 0\n", stdout);
#if defined __WIN32__ || defined _WIN32 || defined _Windows || defined __MSDOS__
    fputs("\nOptions may start with a dash or a slash; the options \"-d0\" and \"/d0\" are\n", stdout);
    fputs("equivalent.\n", stdout);
#endif
    fputs("\nOptions with a value may optionally separate the value from the option letter\n", stdout);
    fputs("with a colon (\":\") or an equal sign (\"=\"). That is, the options \"-d0\", \"-d=0\"\n", stdout);
    fputs("and \"-d:0\" are all equivalent.\n", stdout);
  } /* if */
...
static int operatoradjust(int opertok,symbol *sym,char *opername,int resulttag)
{
  ...
  if ((oldsym=findglb(tmpname,sGLOBAL))!=NULL) {
    ...
    if ((oldsym->usage & uDEFINE)!=0) {
      ...
      funcdisplayname(errname,sizeof(errname),tmpname);
...
SC_FUNC char *operator_symname(char *symname,char *opername,int tag1,int tag2,int numtags,int resulttag)
{
  ...
  if (opertok=='=')
    (void)sprintf(symname,"%s%s%s",tag2str(tagstr1,resulttag),opername,tag2str(tagstr2,tag1));
  else if (numtags==1 || opertok=='~')
    (void)sprintf(symname,"%s%s",opername,tag2str(tagstr1,tag1));
  else
    (void)sprintf(symname,"%s%s%s",tag2str(tagstr1,tag1),opername,tag2str(tagstr2,tag2));
  return symname;
}
...
SC_FUNC void check_tagmismatch(int formaltag,int actualtag,int allowcoerce,int errline)
{
  if (!matchtag(formaltag,actualtag,allowcoerce)) {
    ...
    if(formaltag!=0) {
      ...
      (void)snprintf(formal_tagname,sizeof(formal_tagname),"\"%s\",", (tagsym!=NULL) ? tagsym->name : "-unknown-");
    } /* if */
    if(actualtag!=0) {
      tagsym=find_tag_byval(actualtag);
      (void)snprintf(actual_tagname,sizeof(actual_tagname),"\"%s\"",(tagsym!=NULL) ? tagsym->name : "-unknown-");
    } /* if */
    if(errline>0)
  ...
  if (!checktag(formaltags, numtags, actualtag)) {
    ...
    for (i=0; i<numtags; i++) {      
      if(formaltags[i]!=0) {
        ...
        size=snprintf(formal_tagnames,sizeof(formal_tagnames),
          "%s\"%s\"",formal_tagnames,(tagsym!=NULL) ? tagsym->name : "-unknown-");
    ...
    if(actualtag!=0) {
      tagsym=find_tag_byval(actualtag);
      (void)snprintf(actual_tagname,sizeof(actual_tagname),
        "\"%s\"",(tagsym!=NULL) ? tagsym->name : "-unknown-");
    } /* if */  
    if(errline>0)
      errorset(sSETPOS,errline);
...
SC_FUNC char *funcdisplayname(char *dest,size_t len,char *funcname)
{
  ...
  if (unary) {
    (void)snprintf(dest,len,"operator%s(%s:)",opname,tagsym[1]->name);
  } else {
    ...
    if (opname[0]=='=' && opname[1]=='\0')
      (void)snprintf(dest,len,"%s:operator%s(%s:)",tagsym[0]->name,opname,tagsym[1]->name);
    else
      (void)snprintf(dest,len,"operator%s(%s:,%s:)",opname,tagsym[0]->name,tagsym[1]->name);
  } /* if */
  return dest;
}
...
static void make_report(symbol *root,FILE *log,char *sourcefile)
{
  ...
  for (sym=root->next; sym!=NULL; sym=sym->next) {
    ...
    if ((sym->usage & uREAD)==0)
      continue;
    fprintf(log,"\t\t<member name=\"T:%s\" value=\"%"PRIdC"\">\n",funcdisplayname(symname,sizeof(symname),sym->name),sym->addr);
    if (sym->tag!=0) {
    ...
    if ((enumroot=sym->dim.enumlist)!=NULL) {
      ...
      while (cur!=NULL) {
        fprintf(log,"\t\t\t<member name=\"C:%s\" value=\"%"PRIdC"\">\n",funcdisplayname(symname,sizeof(symname),cur->name),cur->value);
    ...
    for (i=0; i<sym->numrefers; i++) {
      if ((ref=sym->refer[i])!=NULL)
        fprintf(log,"\t\t\t<referrer name=\"%s\"/>\n",xmlencode(symname,funcdisplayname(symname,sizeof(symname),ref->name)));
    } /* for */
    if (sym->documentation!=NULL)
      fprintf(log,"\t\t\t%s\n",sym->documentation);
    fprintf(log,"\t\t</member>\n");
...
static void SC_FASTCALL emit_invalid_token(int expected_token,int found_token)
{
  ...
  if (found_token<tFIRST) {
    (void)snprintf(s,sizeof(s),"%c",(char)found_token);
...
fetchtok:
  ...
  case '-':
    ...
    invalid_token_neg:
      (void)snprintf(ival,sizeof(ival),"-%s",str);
...
#if PAWN_CELL_SIZE==16
    char ival[7];
    (void)snprintf(ival,sizeof(ival),"%ld",(long)p->value.ucell);
#elif PAWN_CELL_SIZE==32
    char ival[12];
    (void)snprintf(ival,sizeof(ival),"%ld",(long)p->value.ucell);
#elif PAWN_CELL_SIZE==64
    char ival[21];
    (void)snprintf(ival,sizeof(ival),"%lld",(long long)p->value.ucell);
#else
  #error Unsupported cell size
#endif
...
static void doreturn(void)
{
  ...
  if (!matchtoken(tTERM)) {
    ...
    if ((rettype & uRETVALUE)!=0 && (curfunc->flags & flagNAKED)==0) {
      char symname[2*sNAMEMAX+16];      /* allow space for user defined operators */
      assert(curfunc!=NULL);
      funcdisplayname(symname,sizeof(symname),curfunc->name);
      error(209,symname);               /* function should return a value */
    } /* if */
  ...
  if (flabel>=0)
    ...
    if (sc_status==statFIRST) {
      ...
      if ((doc=(char*)malloc(length*sizeof(char)))!=NULL) {
        do {
          (void)sprintf(doc,"<transition target=\"%s\"",state->name);
          if (listid>=0) {
            /* get the source state */
            stateindex=state_listitem(listid,listindex);
            state=state_findid(stateindex);
            assert(state!=NULL);
            (void)sprintf(doc+strlen(doc)," source=\"%s\"",state->name);
          } /* if */
